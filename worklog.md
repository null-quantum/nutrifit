...
---
Task ID: A2-LANDING
Agent: premium-landing-animations
Task: Apply premium scroll animations to all landing sections

Work Log:
- `src/components/landing/LandingPage.tsx` — Added `<ScrollProgressBar />` (gradient cyan→teal→emerald, fixed top, z-100) above the Navbar so a thin progress bar reflects scroll position across the whole landing page.
- `src/components/landing/HeroSection.tsx` — Headline now uses `TextReveal` mode="words" (two-line, staggered 0.08s, sliding up from behind an overflow-hidden mask). Subhead wrapped in `ScrollReveal` (direction=up, delay 1.2s). CTA buttons wrapped in `StaggerContainer`+`StaggerItem` (stagger 0.12s). The 5 mini feature cards use `StaggerContainer`+`StaggerItem` and each card is now a `MagneticCard` (3D tilt toward cursor + lift). The background apple image is wrapped in `ParallaxLayer` (speed 0.4) AND `Floating` (distance 20, duration 6) so it both parallaxes on scroll and gently bobs; the existing mouse-parallax wrapper and pulsing drop-shadow glow were preserved. Ambient orbiting particle orb converted to a `Floating` element. Easing upgraded to `EASE.out`.
- `src/components/landing/BMIShowcaseSection.tsx` — Left column wrapped in `ScrollReveal` (direction=left). "BODY MASS INDEX" h1 uses `TextReveal` mode="block" (blur-to-sharp). Sub-headline uses `TextReveal` mode="words". Body copy + button each wrapped in `ScrollReveal` with staggered delays. Right-side BMI illustration wrapped in `ScrollReveal` (direction=right) → `ParallaxLayer` (speed 0.25) → `Floating` (distance 20, duration 5) → image with scale-in. Bottom 4 feature cards use `StaggerContainer`+`StaggerItem` and each card is a `MagneticCard` (strength 8, lift 8). Hover scale on icon now uses `EASE.out`.
- `src/components/landing/CoreFeaturesSection.tsx` — Removed the local containerVariants/itemVariants in favor of `StaggerContainer`+`StaggerItem` (stagger 0.12s). Section header uses `TextReveal` mode="block" for eyebrow and mode="words" for the main h3, plus a `ShimmerLine` accent. Each of the 6 feature cards is now a `MagneticCard` (strength 6, lift 10) preserving the full image/hover-reveal inner markup. Existing image scale-on-hover and gradient overlays untouched.
- `src/components/landing/AlternatingFeaturesSection.tsx` — "How NutriFit Works" headline uses `TextReveal` mode="words". Description wrapped in `ScrollReveal`. The 4 step rows and ArrowDown dividers wrapped in `StaggerContainer`+`StaggerItem` (direction=left, stagger 0.12s). Right-side decorative blur orb converted to `Floating`. Vitamin capsule image wrapped in `ParallaxLayer` (speed 0.35). Added a `ShimmerLine` accent at the bottom.
- `src/components/landing/PricingSection.tsx` — Eyebrow + headline use `TextReveal` (block + words). Subhead wrapped in `ScrollReveal`. Background glow converted to `Floating`. Cards wrapped in `StaggerContainer`+`StaggerItem` (stagger 0.14s), each card is a `MagneticCard` (strength 6, lift 8). The popular "Plus" plan gets an additional `Floating` glow layer behind it (the subtle floating glow requested). Added a `ShimmerLine` divider below the grid. All existing pricing data, button styles, and the Most Popular badge preserved.
- `src/components/landing/AboutSection.tsx` — Left manifesto column wrapped in `ScrollReveal` (direction=left). Eyebrow + h3 headline use `TextReveal`. Added `ShimmerLine` accent. Right-side values grid uses `StaggerContainer`+`StaggerItem` (direction=right, stagger 0.15s). The two ambient rose/red glow orbs converted to `Floating` elements with different durations and a delay so they drift independently.
- `src/components/landing/ContactSection.tsx` — Left typography column wrapped in `ScrollReveal` (direction=left). Headline uses `TextReveal` mode="words" + the word "blueprint." uses `GradientText` wrapped around a `TextReveal` mode="block" (animated rose→pink→fuchsia gradient shift). Contact info items use `StaggerContainer`+`StaggerItem`. Right-side glass form wrapped in `ScrollReveal` (direction=right, delay 0.2s). Form heading uses `TextReveal` mode="block". Background lighting orbs converted to `Floating`. All input fields, labels, and the Send button preserved as-is.
- `src/components/landing/CallToActionSection.tsx` — Eyebrow badge wrapped in `ScrollReveal`. Headline split into two `TextReveal` mode="words" blocks with staggered delays. Subhead + button each wrapped in `ScrollReveal` with sequential delays. Center glow orb converted to `Floating`. Added `ShimmerLine` accent below the CTA.
- `src/components/landing/Footer.tsx` — Park background image now wrapped in `ParallaxLayer` (speed 0.2) for subtle depth. Top trust-metrics row uses `StaggerContainer`+`StaggerItem`. Main content columns wrapped in `ScrollReveal` (fade). Bottom copyright row wrapped in `ScrollReveal` (fade, delay 0.2s). All links, social icons, newsletter input, and back-to-top button logic preserved.

Stage Summary:
- Every landing section now animates in on scroll using `whileInView` with `viewport={{ once: true, margin: "-80px" }}` via the shared primitives — content reveals as the user scrolls TO each section rather than all at once on load.
- All major headlines (Hero, BMI, Core Features, Pricing, About, Contact, CTA) reveal word-by-word via `TextReveal` mode="words" with a mask-slide effect; BMI eyebrow + section eyebrows use mode="block" (blur-to-sharp).
- Feature cards (Hero mini-cards, BMI feature cards, Core Features grid, Pricing tiers) all use `MagneticCard` for 3D cursor tilt + hover lift.
- Parallax depth added to: Hero apple, BMI illustration, Alternating Features vitamin capsule, Footer park background.
- Ambient `Floating` orbs added to Hero, BMI (via image), Pricing glow, About (2 orbs), Contact (2 orbs), CTA glow, Alternating Features decor — each with distinct duration/delay for organic motion.
- Premium `GradientText` (animated color shift) applied to the Contact "blueprint." headline.
- `ShimmerLine` accents added under Core Features, Alternating Features, Pricing, About, CTA section dividers.
- `ScrollProgressBar` mounted at the top of the landing page.
- All transitions use `EASE.out` (cubic-bezier [0.16, 1, 0.3, 1]); never linear.
- All existing logic preserved: `useAuthStore` setView calls in Hero/BMI/Pricing/CTA, anchor links (#features, #pricing, etc.), form inputs in Contact, newsletter input in Footer, scrollToTop in Footer, image sources, gradient classes, and all text content.
- Lint: 0 errors, 0 new warnings (only pre-existing `no-page-custom-font` warning in `src/app/layout.tsx` which is outside the landing scope).
- Files changed: 10 (LandingPage, HeroSection, BMIShowcaseSection, CoreFeaturesSection, AlternatingFeaturesSection, PricingSection, AboutSection, ContactSection, CallToActionSection, Footer). No files outside `src/components/landing/` were modified.
